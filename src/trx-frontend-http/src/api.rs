...
#[get("/status")]
pub async fn status_api(state: web::Data<watch::Receiver<RigState>>) -> Result<impl Responder, Error> {
...
#[post("/toggle_vfo")]
pub async fn toggle_vfo(rig_tx: web::Data<mpsc::Sender<RigRequest>>) -> Result<HttpResponse, Error> {
    send_command(&rig_tx, RigCommand::ToggleVfo).await
}

#[post("/lock")]
pub async fn lock_panel(rig_tx: web::Data<mpsc::Sender<RigRequest>>) -> Result<HttpResponse, Error> {
    send_command(&rig_tx, RigCommand::Lock).await
}

#[post("/unlock")]
pub async fn unlock_panel(rig_tx: web::Data<mpsc::Sender<RigRequest>>) -> Result<HttpResponse, Error> {
...
async fn send_command(
    ...
    rig_tx
        ...
        .await
        .map_err(|e| actix_web::error::ErrorInternalServerError(format!("failed to send to rig: {e:?}")))?;
...
async fn wait_for_view(mut rx: watch::Receiver<RigState>) -> Result<RigSnapshot, actix_web::Error> {
    ...
    Err(actix_web::error::ErrorInternalServerError("Rig state unavailable"))
}
