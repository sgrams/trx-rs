...
async fn main() -> DynResult<()> {
    ...
    if matches!(cli.frontend, FrontendKind::Http) {
        HttpFrontend::spawn_frontend(http_state_rx, tx.clone());
    }
...
async fn rig_task(
    ...
    let mut trx: Box<dyn RigCat> = build_rig(rig_kind, access)?;
    info!("Rig backend ready");

    let rig_info = trx.info().clone();
    state.rig_info = Some(rig_info);
    // Prime VFO state (A/B) by toggling twice so we have both frequencies cached.
    if let Err(e) = prime_vfo_state(&mut trx, &mut state).await {
    ...
    // Handle incoming requests and periodic CAT polling in the same task.
    loop {
        tokio::select! {
            maybe_req = rx.recv() => {
                ...
                let RigRequest { cmd, respond_to } = req;

                let result = match cmd {
                    RigCommand::GetSnapshot => match refresh_state_with_retry(&mut trx, &mut state, 2).await {
                        Ok(()) => {
                            let _ = state_tx.send(state.clone());
                            snapshot_from(&state)
                        }
                    ...
                    RigCommand::SetFreq(freq) => {
                        ...
                        if let Err(e) = trx.set_freq(freq).await {
                    ...
                    RigCommand::SetMode(mode) => {
                        ...
                        if let Err(e) = trx.set_mode(mode.clone()).await {
                    ...
                    RigCommand::SetPtt(ptt) => {
                        ...
                        if let Err(e) = trx.set_ptt(ptt).await {
                    ...
                    RigCommand::PowerOff => {
                        ...
                        if let Err(e) = trx.power_off().await {
                    ...
                    RigCommand::ToggleVfo => {
                        ...
                        if let Err(e) = trx.toggle_vfo().await {
                            error!("Failed to send CAT ToggleVfo: {:?}", e);
                            Err(RigError(format!("CAT error: {}", e)))
                        } else {
                            // Give the rig a beat to switch VFO then refresh freq/mode.
                    ...
                    RigCommand::GetTxLimit => match trx.get_tx_limit().await {
                        Ok(limit) => {
                            state
                                .status
                                .tx
                    ...
                    RigCommand::SetTxLimit(limit) => match trx.set_tx_limit(limit).await {
                        Ok(()) => {
                            state
                                .status
                                .tx
                ...
                let response: RigResult<RigSnapshot> = result;

                // Ignore error if the client has already disconnected.
...
async fn refresh_state_from_cat(trx: &mut Box<dyn RigCat>, state: &mut RigState) -> DynResult<()> {
    ...
    if let Ok(meter) = trx.get_signal_strength().await {
